...
/* ---------- Camera capture for NIN slip ---------- */
function SlipCamera({ onCapture }) {
    ...
    <div className="space-y-3">
      ...
      {phase === "recording" && (
        <p className="text-center text-xs text-muted-foreground">
          Follow the prompts. Keep your face centered.
        </p>
      )}
      {phase === "done" && (
...
export default function KycPage({ onBack }) {
  ...
  const steps = ["ID Type", "NIN + OCR", "Liveness", "Fraud Check", "Success"];
  ...
  useEffect(() => {
    ...
    if (user.cooldown_until) {
      ...
      if (expiry > Date.now()) {
        setCooldownInfo({ until: user.cooldown_until, remaining: expiry - Date.now() });
      }
    }
    ...
    return (
      <div className="mx-auto flex max-w-xl flex-col items-center px-4 py-16 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          This account has been permanently banned due to repeated verification failures.
          If you believe this is an error, contact support@directnest.com.ng
        </p>
    ...
    return (
      <div className="mx-auto flex max-w-xl flex-col items-center px-4 py-16 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          ...
          You can chat with owners and book inspections directly.
        </p>
  ...
  async function simulateOCR() {
    ...
    // In production: call edge function for AWS Textract / Google Vision OCR
    // For now: pre-fill with test data if NIN is the test NIN
    if (nin === "12345678901") {
  ...
  async function runFaceMatch() {
    ...
    try {
      ...
      // Fallback: if face-api fails (e.g. models don't load on mobile), use a simulated score
      setError("Face detection unavailable. Using test mode.");
      setFaceMatchScore(0.85);
    } finally {
      setFaceMatchLoading(false);
    }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // 4b. Check rate limit: 1 attempt per NIN per 24hrs
      ...
      let selfieUrl = null;
      try {
        idPhotoUrl = await uploadFile(slipPhoto.file, KYC_BUCKET, folder);
        selfieUrl = await uploadFile(selfieData.file, KYC_BUCKET, folder);
      } catch {
        // Storage upload may fail in sandbox — continue with verification
      }
      ...
      if (!passed) {
        ...
        if (newFailed >= 3) {
          ...
          await supabase.from("kyc_blacklist").insert({
            ...
            nin_hash: ninHash,
            nin: nin.trim(),
            reason: "3 failed face match attempts",
          });

          setBlacklisted(true);
        } else if (newFailed >= 2) {
          // 24hr cooldown after 2nd fail
      ...
      // Step 5: Success — save to verified_users
      await supabase.from("verified_users").insert({
        user_id: userId,
        nin_hash: ninHash,
        full_name: ocrData.name || "Verified User",
        status: "verified",
      });
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
          ...
          <section className="space-y-4 rounded-xl border border-border bg-card p-5">
            ...
            <div className="grid gap-3">
              ...
              >
                ...
                />
                <div>
                  <p className="font-semibold">NIN Verification</p>
                  <p className="text-xs text-muted-foreground">National Identity Number — Active</p>
                </div>
                <Badge className="ml-auto bg-green-50 text-green-700 ring-1 ring-green-200">
                  Active
                </Badge>
              </label>

              {/* BVN — Disabled / Coming Soon */}
              <div className="group relative">
                <label
                  className={cn(
                    "flex cursor-not-allowed items-start gap-3 rounded-lg border-2 border-border bg-secondary/30 p-4 opacity-70 transition"
                  )}
                >
                  <input
                    type="radio"
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Available at launch for instant verification
                    </p>
                  </div>
                ...
                {/* Tooltip */}
                <div className="pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 rounded-md bg-foreground px-2 py-1 text-xs font-medium text-background opacity-0 transition group-hover:opacity-100">
                  BVN verification will be available at launch
                </div>
              </div>
            </div>
          </section>
      ...
      {/* Step 1: NIN Capture + OCR */}
      {step === 1 && (
        <div className="space-y-6">
          <section className="space-y-4 rounded-xl border border-border bg-card p-5">
            <h2 className="flex items-center gap-2 text-base font-semibold">
              <IdCard className="size-4 text-primary" /> NIN Number
            </h2>
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {nin.length === 11 ? (
                  <span className="font-medium text-green-600">Valid format</span>
                ) : (
                  <span>Must be exactly 11 digits ({nin.length}/11)</span>
                )}
              </p>
            </div>
          </section>
          ...
          {nin.length === 11 && (
            <section className="space-y-4 rounded-xl border border-border bg-card p-5">
              <h2 className="flex items-center gap-2 text-base font-semibold">
                <Camera className="size-4 text-primary" /> Capture NIN Slip
              </h2>
              <p className="text-sm text-muted-foreground">
                Use your camera to photograph your NIN slip. We'll use OCR to auto-fill your details.
              </p>
              {!slipPhoto ? (
                <SlipCamera onCapture={(data) => { setSlipPhoto(data); simulateOCR(); }} />
              ) : (
                <div className="space-y-3">
                  <div className="relative overflow-hidden rounded-lg border border-border">
                    ...
                    <button
                      type="button"
                      onClick={() => { setSlipPhoto(null); setOcrData({ name: "", dob: "", gender: "" }); }}
                  ...
                  {ocrLoading ? (
                    <div className="flex items-center justify-center gap-2 py-3 text-sm text-muted-foreground">
                      <Loader2 className="size-4 animate-spin" /> Extracting details with OCR…
                    </div>
                  ) : (
                    <div className="space-y-3">
                      <div className="rounded-lg border border-green-200 bg-green-50 p-3">
                        <p className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-green-700">
                          <Sparkles className="size-3.5" /> OCR extracted — review and edit if needed
                        </p>
                        <div className="grid gap-2 sm:grid-cols-2">
                          <div>
                            <Label htmlFor="ocrName">Full Name</Label>
                            <Input
                              id="ocrName"
                              value={ocrData.name}
                          ...
                          <div>
                            <Label htmlFor="ocrDob">Date of Birth</Label>
                            <Input
                              id="ocrDob"
                              value={ocrData.dob}
                          ...
                          <div>
                            <Label htmlFor="ocrGender">Gender</Label>
                            <Input
                              id="ocrGender"
                              value={ocrData.gender}
                              onChange={(e) => setOcrData((p) => ({ ...p, gender: e.target.value }))}
                              placeholder="Gender"
                            />
                          </div>
                        </div>
          ...
          <div className="flex gap-2">
            ...
            <Button
              onClick={() => {
                setError("");
                if (!/^[0-9]{11}$/.test(nin.trim())) {
                  setError("Please enter a valid 11-digit NIN.");
                  return;
                }
                if (!slipPhoto) {
                  setError("Please capture a photo of your NIN slip.");
                  return;
                }
                if (!ocrData.name.trim()) {
                  setError("Please enter or confirm your name from the OCR.");
                  return;
                }
                setStep(2);
              }}
              disabled={nin.length !== 11 || !slipPhoto || !ocrData.name}
      ...
      {step === 2 && (
        <div className="space-y-6">
          <section className="space-y-4 rounded-xl border border-border bg-card p-5">
            <h2 className="flex items-center gap-2 text-base font-semibold">
              <ScanFace className="size-4 text-primary" /> Liveness + Face Match
            </h2>
            <p className="text-sm text-muted-foreground">
              We'll capture a quick selfie following the prompts:{" "}
              <span className="font-medium text-foreground">Blink, Turn Left, Smile</span>.
              Your selfie will be compared to your NIN slip photo.
            </p>
            ...
            {!selfieData ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 rounded-lg border border-green-200 bg-green-50 p-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-green-700">Selfie captured</p>
                    <p className="text-xs text-green-600">Ready for face match</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <img src={slipPhoto.url} alt="NIN Slip" className="size-20 rounded-md border border-border object-cover" />
                  <div className="flex items-center text-muted-foreground">→</div>
                  <img src={selfieData.url} alt="Selfie" className="size-20 rounded-md border border-border object-cover" />
                </div>
                <Button variant="outline" onClick={() => { setSelfieData(null); setFaceMatchScore(null); }} className="w-full">
                  Retake Selfie
                </Button>
              </div>
            )}
          ...
          {selfieData && faceMatchScore === null && !faceMatchLoading && (
            ...
            >
              <ScanFace className="size-4" /> Run Face Match
            </Button>
          )}

          {faceMatchLoading && (
            <div className="flex items-center justify-center gap-2 py-3 text-sm text-muted-foreground">
              <Loader2 className="size-4 animate-spin" /> Comparing faces…
            </div>
          )}
            ...
            <div className={cn(
              ...
              <div>
                <p className={cn(
                  ...
                  faceMatchScore > 0.7 ? "text-green-700" : "text-red-700"
                )}>
                  Face Match Score: {(faceMatchScore * 100).toFixed(1)}%
                  {faceMatchScore > 0.7 ? " — Pass" : " — Below threshold (70%)"}
                </p>
                <p className="text-xs text-muted-foreground">
                  Threshold: 70% (0.7)
                </p>
          ...
          <div className="rounded-lg border border-dashed border-border p-3">
            ...
            {showTestControls && (
              <div className="mt-3 flex flex-wrap gap-2">
                <Button
                  variant="outline"
                  onClick={() => { setTestMode("pass"); setFaceMatchScore(0.85); }}
                  className="text-xs"
                >
                  Simulate Pass (85%)
                </Button>
                <Button
                  variant="outline"
                  onClick={() => { setTestMode("fail"); setFaceMatchScore(0.3); }}
                  className="text-xs"
                >
                  Simulate Fail (30%)
                </Button>
                <p className="w-full text-xs text-muted-foreground">
                  Test NIN: 12345678901, Name: TEST USER
                </p>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Button
              onClick={() => {
                setError("");
                if (!selfieData) { setError("Please capture a selfie."); return; }
                if (faceMatchScore === null) { setError("Please run the face match."); return; }
                setStep(3);
              }}
              disabled={!selfieData || faceMatchScore === null}
              className="flex-1 bg-green-600 text-white hover:bg-green-700"
            >
              Continue <ArrowRight className="size-4" />
            </Button>
      ...
      {/* Step 3: Fraud Checks + Submit */}
      {step === 3 && (
        <div className="space-y-6">
          <section className="space-y-4 rounded-xl border border-border bg-card p-5">
            <h2 className="flex items-center gap-2 text-base font-semibold">
              <Fingerprint className="size-4 text-primary" /> Fraud Checks
            </h2>
            <div className="space-y-3">
              <div className="flex items-center gap-3 rounded-lg border border-border p-3">
                <CheckCircle2 className="size-5 text-green-600" />
                <div>
                  <p className="text-sm font-medium">NIN Blacklist Check</p>
                  <p className="text-xs text-muted-foreground">Checking against blocked IDs</p>
                </div>
              ...
              <div className="flex items-center gap-3 rounded-lg border border-border p-3">
                <CheckCircle2 className="size-5 text-green-600" />
                <div>
                  <p className="text-sm font-medium">Rate Limit Check</p>
                  <p className="text-xs text-muted-foreground">1 attempt per NIN per 24 hours</p>
                </div>
              ...
              <div className="flex items-center gap-3 rounded-lg border p-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Score: {(faceMatchScore * 100).toFixed(1)}% (threshold: 70%)
                  </p>
                </div>
                <Badge className={cn(
                  "ml-auto",
